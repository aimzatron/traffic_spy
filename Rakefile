...
namespace :sanitation do
  desc "Check line length & whitespce with Cane"
  task :lines do
    puts ""
  ...
  desc "Check method length with Reed"
  task :methods do
    puts ""
    puts"== using reek to check method length =="
    system("reek -n lib/**/*.rb | grep -v ' 0 warnings '")
    puts "== done checking method length =="
    puts ""
  end

  desc "Check both line length and method lenght"
